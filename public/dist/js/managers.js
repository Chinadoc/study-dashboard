...
const DataPortability = {
    syncAllToCloud: async function () {
        ...
        try {
            // Use user-scoped inventory cache key for consistency
            const userId = currentUser?.sub || currentUser?.id;
            const inventoryCacheKey = userId ? `eurokeys_inventory_${userId}` : 'eurokeys_inventory';
            const inventory = JSON.parse(localStorage.getItem(inventoryCacheKey) || '[]');
