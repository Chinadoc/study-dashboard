...
// ============================================================
// D1 Helper — queries remote D1 via temp SQL files (avoids shell escaping issues)
// ============================================================
function queryD1(sql: string): any[] {
    const tmpFile = path.join(tmpdir(), `vi_query_${Date.now()}_${tmpCounter++}.sql`);
    try {
        writeFileSync(tmpFile, sql);
        const output = execSync(
            `npx wrangler d1 execute locksmith-db --remote --json --file=${tmpFile}`,
...
function executeD1(sql: string): void {
    ...
    try {
        ...
        execSync(
            ...
            { cwd: API_DIR, maxBuffer: 10 * 1024 * 1024, timeout: 120000 }
        );
    } catch (err: any) {
...
async function main() {
    ...
    SELECT DISTINCT vy.make, vy.model, 
           ...
           vy.page_id,
           vy.chip_type, vy.battery, vy.lishi
  ...
  `);
    ...
    for (const v of vehicles) {
        ...
        const finalChip = v.chip_type || bitting?.v_chip || chipConsensus.value;
        const finalBattery = v.battery || batteryConsensus.value;
        const finalLishi = v.lishi || bitting?.lishi_tool;
