#!/usr/bin/env python3
"""Upload optimized vehicle thumbnails to R2 bucket"""
...
def main():
    if not THUMB_DIR.exists():
        print(f"Error: {THUMB_DIR} not found")
        sys.exit(1)
    
    total = 0
    success = 0
    
    for make_dir in sorted(THUMB_DIR.iterdir()):
        if not make_dir.is_dir():
            continue
        
        make = make_dir.name
        images = list(make_dir.glob("*.png"))
        print(f"\n{make.upper()}: {len(images)} thumbnails")
        
        for img_path in sorted(images):
            r2_key = f"vehicles_thumb/{make}/{img_path.name}"
            total += 1
    ...
    print(f"\n=== COMPLETE: {success}/{total} thumbnails uploaded ===")
