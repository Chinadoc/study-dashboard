...
Respond with ONLY the title, nothing else.`;
        ...
        // POST /api/ai/extract-license - Extract license info from uploaded document image
        if (path === "/api/ai/extract-license" && request.method === "POST") {
          try {
            const body: any = await request.json();
            ...
            // Check for Gemini API key (required for multimodal)
            ...
            const extractionPrompt = `You are analyzing an image of a professional license, certification, permit, or insurance document.

Extract the following information from this document. If a field is not visible or unclear, use null.

Respond ONLY with valid JSON in this exact format:
{
  "name": "Full name of the license/certification",
  "type": "state_license" | "business_license" | "certification" | "vehicle_access" | "tool_subscription" | "oem_access" | "insurance",
  "obtainedDate": "YYYY-MM-DD or null",
  "expirationDate": "YYYY-MM-DD or null",
  "price": number or null,
  "notes": "License number, issuing authority, or other important details",
  "confidence": {
    "name": 0.0-1.0,
    "type": 0.0-1.0,
    "obtainedDate": 0.0-1.0,
...
- state_license: State-issued locksmith or trade license
- business_license: City/county business permit
- certification: ALOA, CRL, manufacturer certifications
- vehicle_access: NASTF, AWR, NHTSA programs
- tool_subscription: Autel, Lonsdor, SmartPro subscriptions
- oem_access: GM TIS2Web, Toyota Techstream, FCA wiTECH
- insurance: Liability, bonding, tools coverage

Be thorough - look for dates in headers, footers, stamps, and stickers.`;
            ...
            const fields = {
              ...
              type: extracted.type || "state_license",
              obtainedDate: extracted.obtainedDate || null,
            ...
            return corsResponse(request, JSON.stringify({ fields, confidence }));

          } catch (err: any) {
            console.error("License extraction error:", err);
            return corsResponse(request, JSON.stringify({
