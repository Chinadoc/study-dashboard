...
WHERE keyway IN ('HY16', 'HYN14R', 'HY14R') OR 
      (lishi_tool IS NULL AND vehicle_id IN (
          SELECT id FROM vehicles WHERE make IN ('Hyundai', 'Kia')
      ));
...
WHERE keyway = 'HON66' OR
      (lishi_tool IS NULL AND vehicle_id IN (
          SELECT id FROM vehicles WHERE make IN ('Honda', 'Acura')
      ));
...
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make IN ('Toyota', 'Lexus')
...
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make IN ('Ford', 'Lincoln')
...
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make IN ('Chevrolet', 'Buick', 'GMC', 'Cadillac')
) AND lishi_tool IS NULL;
...
SET lishi_tool = 'CY24'
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make IN ('Chrysler', 'Dodge', 'Jeep', 'Ram')
) AND lishi_tool IS NULL;
...
WHERE keyway IN ('HU92', 'HU100R') OR
      vehicle_id IN (
          SELECT id FROM vehicles WHERE make = 'BMW'
      ) AND lishi_tool IS NULL;
...
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make IN ('Nissan', 'Infiniti')
) AND lishi_tool IS NULL;
...
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make = 'Mazda'
) AND lishi_tool IS NULL;
...
SET lishi_tool = 'HU66'
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make IN ('Volkswagen', 'Audi')
...
SET lishi_tool = 'HU64'
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make = 'Mercedes-Benz'
) AND lishi_tool IS NULL;
...
SET lishi_tool = 'NSN14'
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make = 'Subaru'
) AND lishi_tool IS NULL;
...
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make = 'Mitsubishi'
) AND lishi_tool IS NULL;
...
SET lishi_tool = 'HU101'
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make IN ('Jaguar', 'Land Rover')
) AND lishi_tool IS NULL;
...
SET lishi_tool = 'HU101'
WHERE vehicle_id IN (
    SELECT id FROM vehicles WHERE make = 'Volvo'
) AND lishi_tool IS NULL;
