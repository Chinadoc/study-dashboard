...
interface VehicleSearchResult {
    ...
    model: string;
    year: string | null;
    tools: Array<{ name: string; functions: string[]; ecu: string[] }>;
    score: number;
}
...
const MAKE_BLACKLIST = new Set([
    'Models', 'Notes',
    // Tablet spec artifacts (Launch X431 V+ / X-PROG3 parse contamination)
    '1.8Ghz Octa-Core', '10.1"', '1920*1200', '32Gb', '3Gb',
    '5Mb Front', '7000Mah', '8Mp Rear', 'Android™ 9.0',
    // Non-automotive entries
    'Five Signs(Ufo)',
]);
const MAKE_NORMALIZE: Record<string, string> = {
    ...
    'Bmw (China)': 'BMW',
    'Skoda (Svw)': 'Skoda',
    'Qj-Benelli': 'Qj Benelli',
    'Can-Am (Brp)': 'Can-Am',
    'Sea-Doo (Brp)': 'Sea-Doo',
    'Ski-Doo (Brp)': 'Ski-Doo',
    'Lynx (Brp)': 'Lynx',
    // Unicode middle-dot variants (from certain scraped sources)
    'Alfa\u00b7Romeo': 'Alfa Romeo',
    'Aston\u00b7Martin': 'Aston Martin',
};
const normalizeMake = (m: string): string | null => {
    if (MAKE_BLACKLIST.has(m)) return null;
    // Safety net: catch any spec-string contamination (starts with digit, contains Ghz/Gb/Mah)
    if (/^\d+[\.\*"]|[Gg][Hh]z|[Mm][Aa][Hh]$|^Android/i.test(m)) return null;
    // Replace unicode middle-dots with spaces before lookup
    const cleaned = m.replace(/\u00b7/g, ' ');
    return MAKE_NORMALIZE[m] || MAKE_NORMALIZE[cleaned] || cleaned;
};
...
export default function ToolCoverageHeatmap() {
    ...
    const [vehicleCoverage, setVehicleCoverage] = useState<Record<string, any> | null>(null);
    ...
    const { coverageMap, yearCoverageMap, allMakes, stats } = useMemo(() => {
        ...
        family.expansions.forEach((exp, idx) => {
            ...
            for (const rawMake of exp.unlocks_makes) {
                ...
                if (!yearMap[make]) yearMap[make] = {};
                for (const bucket of YEAR_BUCKETS) {
                    // Reality Mask: only fill years the vehicle actually existed
                    let bucketHasValidYear = false;
                    for (let y = bucket.start; y <= bucket.end; y++) {
                        if (!isYearValid(make, y)) continue; // Skip phantom years
                        bucketHasValidYear = true;
                        const ys = String(y);
                        if (!yearMap[make][ys]) {
                    ...
                    if (!bucketHasValidYear) continue; // Skip entire bucket
                    if (!map[make][bucket.label]) {
    ...
    const parseVehicleQuery = useCallback((query: string) => {
        const tokens = query.trim().toLowerCase().split(/\s+/);
        let year: number | null = null;
        const rest: string[] = [];
        for (const t of tokens) {
            const n = parseInt(t);
            if (t.length === 4 && n >= 1990 && n <= 2030 && !year) {
                year = n;
            } else {
                rest.push(t);
            }
        }
        return { year, tokens: rest };
    }, []);
    ...
    const handleVehicleSearch = useCallback(async () => {
        ...
        const { year, tokens } = parseVehicleQuery(vehicleSearch);
        ...
        for (const [make, makeData] of Object.entries(coverage as Record<string, any>)) {
            ...
            // Check if any token matches the make
            const makeTokenIdx = tokens.findIndex(t => makeLower.includes(t) || t.includes(makeLower));
            if (makeTokenIdx === -1 && tokens.length > 0) continue;

            // Remaining tokens after make = model search
            const modelTokens = tokens.filter((_, i) => i !== makeTokenIdx);
            ...
            for (const [model, modelData] of Object.entries(makeData.models as Record<string, any>)) {
                ...
                // Filter by model tokens if present
                if (modelTokens.length > 0) {
                    const modelMatches = modelTokens.every(t => modelLower.includes(t));
                    if (!modelMatches) continue;
                }

                // Collect tools for this model, filtered by year if specified
                const toolMap = new Map<string, { functions: Set<string>; ecu: Set<string> }>();

                for (const [yearStr, yearData] of Object.entries(modelData as Record<string, any>)) {
                    const yd = yearData as any;
                    // Year filter
                    if (year) {
                        if (yearStr === '_all') {
                            // _all means all years — include
                        } else {
                            const y = parseInt(yearStr);
                            if (!isNaN(y) && y !== year) continue;
                        }
                    }

                    if (yd.tools) {
                        for (const toolName of yd.tools as string[]) {
                            if (!toolMap.has(toolName)) {
                                toolMap.set(toolName, { functions: new Set(), ecu: new Set() });
                            }
                            const entry = toolMap.get(toolName)!;
                            if (yd.functions) (yd.functions as string[]).forEach(f => entry.functions.add(f));
                            if (yd.ecu) (yd.ecu as string[]).forEach(e => entry.ecu.add(e));
                        }
                    }
                }

                if (toolMap.size === 0) continue;

                // Score: exact model match > partial > make-only
                let score = 0;
                if (modelTokens.length > 0 && modelTokens.every(t => modelLower.includes(t))) score += 10;
                if (model !== '_all') score += 5;
                if (year) score += 3;
                score += toolMap.size;
                ...
                results.push({
                    ...
                    tools: Array.from(toolMap.entries()).map(([name, data]) => ({
                        name,
                        functions: Array.from(data.functions).sort(),
                        ecu: Array.from(data.ecu).sort(),
        ...
        setVehicleResults(results.slice(0, 30));
        setVehicleSearchLoading(false);
    }, [vehicleSearch, vehicleCoverage, parseVehicleQuery]);

    // Lens-specific stats (dynamic recalculation per function)
    const lensStats = useMemo(() => {
        if (!activeLens || !allMakes.length) return null;
        let verified = 0, unverified = 0, expansion = 0, total = 0;
        for (const make of allMakes) {
            for (const bucket of YEAR_BUCKETS) {
                total++;
                const c = coverageMap[make]?.[bucket.label];
                if (!c) continue;
                if (!c.functions.includes(activeLens)) continue;
                if (c.source === 'expansion') expansion++;
                else if (c.confidence === 'claimed') unverified++;
                else verified++;
            }
        }
        const litTotal = verified + unverified + expansion;
        return { verified, unverified, expansion, total, litTotal, pct: total > 0 ? Math.round((litTotal / total) * 100) : 0 };
    }, [activeLens, allMakes, coverageMap]);
            ...
            <div className="border-b border-zinc-800 bg-gradient-to-b from-zinc-900 to-zinc-950">
                <div className="max-w-[1600px] mx-auto px-4 py-6">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <Link
                            href="/tools/brands"
                            className="text-xs text-zinc-500 hover:text-purple-300 transition-colors border border-zinc-800 hover:border-purple-500/30 px-3 py-1.5 rounded-lg"
                        >
                            Browse All Brands →
                        </Link>
                    </div>
                    ...
                    {/* Vehicle Search */}
                    <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-xl p-4 mb-4">
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                placeholder="e.g. 2020 Ford Fusion, BMW X3, Toyota Camry..."
                        ...
                        {vehicleResults && (
                            <div className="mt-3 max-h-80 overflow-y-auto">
                                {vehicleResults.length === 0 ? (
                                    <p className="text-zinc-500 text-sm">No results found. Try a different make or model.</p>
                                ) : (
                                    <div className="space-y-2">
                                        {vehicleResults.map((r, i) => (
                                            <div key={i} className="bg-zinc-800/50 border border-zinc-700/30 rounded-lg px-4 py-3">
                                                <div className="flex items-center justify-between mb-2">
                                                    <div>
                                                        <span className="text-sm font-bold text-white">{r.make}</span>
                                                        <span className="text-zinc-400 text-sm ml-2">{r.model}</span>
                                                        {r.year && <span className="text-purple-400 text-xs ml-2">({r.year})</span>}
                                                    </div>
                                                    <span className="text-[10px] text-zinc-600">{r.tools.length} tool{r.tools.length !== 1 ? 's' : ''}</span>
                                                </div>
                                                <div className="space-y-1.5">
                                                    {r.tools.map(tool => (
                                                        <div key={tool.name} className="flex items-start gap-2">
                                                            <span className="text-xs font-medium text-purple-300 min-w-[80px] flex-shrink-0">{tool.name}</span>
                                                            <div className="flex flex-wrap gap-1">
                                                                {tool.functions.map(f => {
                                                                    const fd = FUNC_DISPLAY[f];
                                                                    return fd ? (
                                                                        <span key={f} className={`text-[9px] px-1.5 py-0.5 rounded bg-zinc-700/80 ${fd.color}`}>
                                                                            {fd.icon} {fd.label}
                                                                        </span>
                                                                    ) : (
                                                                        <span key={f} className="text-[9px] px-1.5 py-0.5 rounded bg-zinc-700/80 text-zinc-400">
                                                                            {f.replace(/_/g, ' ')}
                                                                        </span>
                                                                    );
                                                                })}
                                                                {tool.ecu.length > 0 && (
                                                                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-500">
                                                                        ECU: {tool.ecu.slice(0, 3).join(', ')}{tool.ecu.length > 3 ? ` +${tool.ecu.length - 3}` : ''}
                                                                    </span>
                                                                )}
                                                            </div>
                                                        </div>
                ...
                <div className="max-w-[1600px] mx-auto px-4 py-4">
                    ...
                    <div className="flex flex-col-reverse lg:grid lg:grid-cols-[320px_1fr] gap-4 items-start">
                        ...
                        <div className={`${showMobilePanel ? 'block' : 'hidden'} lg:block w-full overflow-hidden`} style={{ minWidth: 0, maxWidth: 320 }}>
                            ...
                            {/* Coverage donut — dynamic per lens */}
                            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-3 mb-3 flex items-center gap-3">
                                <div className="relative w-12 h-12 flex-shrink-0">
                                    <svg viewBox="0 0 36 36" className="w-12 h-12 -rotate-90">
                                        <circle cx="18" cy="18" r="15.9" fill="transparent" stroke="#27272a" strokeWidth="2" />
                                        <circle cx="18" cy="18" r="15.9" fill="transparent" stroke="url(#grad)" strokeWidth="2"
                                            strokeDasharray={`${lensStats?.pct ?? coveragePct} ${100 - (lensStats?.pct ?? coveragePct)}`}
                                            strokeLinecap="round" />
                                        <defs>
                                    ...
                                    {lensStats ? (
                                        <>
                                            <div className="text-sm font-semibold text-white">
                                                {FUNC_DISPLAY[activeLens!]?.icon} {FUNC_DISPLAY[activeLens!]?.label} Coverage: {lensStats.pct}%
                                            </div>
                                            <div className="text-[10px] text-zinc-500">
                                                {lensStats.verified} verified · {lensStats.unverified} unverified · {lensStats.expansion} expansion
                                            </div>
                                        </>
                                    ) : (
                                        <>
                                            <div className="text-sm font-semibold text-white">{coveragePct}% Verified Coverage</div>
                                            <div className="text-[10px] text-zinc-500">{stats.explicitCells} verified · {stats.inferredCells} inferred · {stats.claimedCells} claimed</div>
                                        </>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* Legend — contextual based on active lens */}
                            <div className="mt-3 bg-zinc-900/50 border border-zinc-800 rounded-lg p-2">
                                {activeLens ? (
                                    <>
                                        <div className="text-[10px] text-zinc-500 font-medium mb-1">Lens States ({FUNC_DISPLAY[activeLens]?.label})</div>
                                        <div className="flex flex-col gap-1.5 text-[9px]">
                                            <span className="flex items-center gap-1.5">
                                                <span className="w-3 h-3 rounded bg-green-500" /> Verified & Owned
                                            </span>
                                            <span className="flex items-center gap-1.5">
                                                <span className="w-3 h-3 rounded border-2 border-green-500 bg-green-500/15" /> Unverified & Owned
                                            </span>
                                            <span className="flex items-center gap-1.5">
                                                <span className="w-3 h-3 rounded border-2 border-dashed border-zinc-500" /> Expansion (locked)
                                            </span>
                                            <span className="flex items-center gap-1.5">
                                                <span className="w-3 h-3 rounded bg-zinc-800/40" /> Partial miss
                                            </span>
                                            <span className="flex items-center gap-1.5">
                                                <span className="w-3 h-3 rounded bg-zinc-800/40 border border-zinc-700" /> No coverage
                                            </span>
                                        </div>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <div className="flex flex-wrap gap-2 text-[9px] mb-2">
                                            ...
                                            <span className="flex items-center gap-1">
                                                <span className="w-3 h-3 rounded" style={{ background: 'repeating-linear-gradient(45deg, transparent, transparent 2px, rgba(245,158,11,0.25) 2px, rgba(245,158,11,0.25) 4px)', border: '1px solid rgba(245,158,11,0.4)' }} /> Add-on (claimed)
                                            </span>
                                            <span className="flex items-center gap-1">
                                                ...
                                                return (
                                                    <tr key={make} className={`group transition-opacity duration-200 ${isRowDimmed ? 'opacity-20' : ''}`}>
                                                        ...
                                                        {columns.map(col => {
                                                            ...
                                                            if (activeLens) {
                                                                // 5-State Lens Mode (with confidence split)
                                                                const hasLensFunc = cell?.functions.includes(activeLens);
                                                                if (!cell) {
                                                                    // State 5: Total miss
                                                                    cellClass = CELL_COLORS.none;
                                                                } else if (!hasLensFunc) {
                                                                    // State 4: Partial miss (has coverage but not this function)
                                                                    cellClass = 'bg-zinc-800/30';
                                                                } else if (cell.source === 'expansion') {
                                                                    // State 3: Expansion (locked — grey dashed)
                                                                    cellClass = 'bg-transparent border-2 border-dashed border-zinc-500';
                                                                    cellIcon = lensInfo?.icon || '';
                                                                } else if (cell.confidence === 'claimed') {
                                                                    // State 2: Unverified & Owned (hollow border + 15% tint)
                                                                    cellClass = `border-2 ${lc.border}`;
                                                                    cellStyle = { backgroundColor: `color-mix(in srgb, ${lc.bg === 'bg-green-500' ? '#22c55e' : lc.bg === 'bg-red-500' ? '#ef4444' : lc.bg === 'bg-cyan-500' ? '#06b6d4' : lc.bg === 'bg-yellow-500' ? '#eab308' : lc.bg === 'bg-orange-500' ? '#f97316' : lc.bg === 'bg-purple-500' ? '#a855f7' : lc.bg === 'bg-blue-500' ? '#3b82f6' : lc.bg === 'bg-pink-500' ? '#ec4899' : '#f59e0b'} 15%, transparent)` };
                                                                    cellIcon = lensInfo?.icon || '';
                                                                } else {
                                                                    // State 1: Verified & Owned (explicit + inferred — solid fill)
                                                                ...
                                                                if (!cell) {
                                                                    ...
                                                                    // Hatched gold: signals make-level claim, unverified at model level
                                                                    cellClass = 'border border-amber-500/40';
                                                                    cellStyle = {
                                                                        background: 'repeating-linear-gradient(45deg, transparent, transparent 2px, rgba(245,158,11,0.25) 2px, rgba(245,158,11,0.25) 4px)',
                                                                    };
                                                                    isNewExpansion = true;
            ...
            {hoveredCell && tooltipPos && (() => {
                ...
                return (
                    ...
                    >
                        ...
                        {cellData.source === 'expansion' && (
                            <div className="text-amber-400/60 text-[8px] italic mt-0.5">
                                ⚠ Manufacturer-claimed. Verify ECU compatibility before purchase.
                            </div>
                        )}
                        {cellData.confidence === 'inferred' && cellData.source !== 'expansion' && (
                            <div className="text-yellow-400/60 text-[8px] italic mt-0.5">
                                Coverage inferred (manufacturer claims &ldquo;All Years&rdquo;; clipped to known production).
                            </div>
                        )}
